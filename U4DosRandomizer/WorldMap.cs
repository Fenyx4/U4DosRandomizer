...
namespace U4DosRandomizer
{
    public class WorldMap
    {
        ...
        public WorldMap(double[,] worldMapGenerated)
        {
            ...
            _worldMapTiles = MapGeneratedMapToUltimaTiles(_worldMapGenerated);
        }

        private static byte[,] MapGeneratedMapToUltimaTiles(double[,] worldMapDS)
        {
            ...
            for (int x = 0; x < WorldMap.SIZE; x++)
            {
                ...
                {
                    worldMapFlattened[x + y * WorldMap.SIZE] = worldMapDS[x, y];
                }
            }
        ...
        private void AddRivers(Random random)
        {
            ...
            {
                // find shortest path
                var path = Search.GetPath(WorldMap.SIZE, WorldMap.SIZE, new List<Coordinate> { highPoint }, IsCoordinateWater, delegate { return true; });
                paths.Add(path);
